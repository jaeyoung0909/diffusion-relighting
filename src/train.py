...
from utils import image_grid, make_train_dataset
...
# NEW: Import main code from local files
...
def log_validation(
    vae, text_encoder, tokenizer, unet, light_encoder, camera_pose_encoder, 
...
def collate_fn(examples):
    ...
    light_types = torch.stack([example['light_type'] for example in examples])
    camera_pose_types = torch.stack([example['camera_pose_type'] for example in examples])
    ...
    return {
        ...
        "camera_pose_types": camera_pose_types,
    }
...
def main(args):
    ...
    params_to_optimize = list(light_encoder.parameters()) + list(camera_pose_encoder.parameters())
    ...
    light_encoder, camera_pose_encoder, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
        light_encoder, camera_pose_encoder, optimizer, train_dataloader, lr_scheduler
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            ...
            # batch : pixel_value, input_ids, light_type, camera_pose_type
            ...
            encoder_hidden_states += light_encoder(batch["light_types"]).expand(B,L,H)
            encoder_hidden_states += camera_pose_encoder(batch["camera_pose_types"]).expand(B,L,H)
            ...
            if accelerator.sync_gradients:
                params_to_clip = list(light_encoder.parameters()) + list(camera_pose_encoder.parameters())
